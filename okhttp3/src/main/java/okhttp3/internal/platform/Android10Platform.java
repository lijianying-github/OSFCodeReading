...
class Android10Platform extends AndroidPlatform {
    ...
    String getSelectedProtocol(SSLSocket socket) {
        ...
        //反射替代 String alpnResult = socket.getApplicationProtocol();
        String alpnResult = null;
        try {
            Method getApplicationProtocol = socket.getClass().getDeclaredMethod("getApplicationProtocol");
            alpnResult = (String) getApplicationProtocol.invoke(socket);
        } catch (NoSuchMethodException | IllegalAccessException | InvocationTargetException e) {
            e.printStackTrace();
        }
